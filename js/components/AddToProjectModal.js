/**
 * AddToProjectModal.js
 * Modal for adding documents to an existing or new project
 */
...
export class AddToProjectModal extends BaseModal {
  constructor() {
  ...
  /**
   * Open the modal to add documents to a project
   * @param {string[]} documentIds - Document IDs to add
   * @param {Function} onComplete - Callback with result { projectId, projectName, added, total }
   */
  open(documentIds, onComplete) {
  ...
  render() {
    ...
    const docLabel = docCount === 1 ? 'document' : 'documents';
    ...
    this.modalContent.innerHTML = `
      ...
      <div class="modal-body">
        <p class="text-sm text-secondary mb-md">
          Adding <strong>${docCount} ${docLabel}</strong> to:
        </p>
    ...
    return `
      <div class="project-select-option ${isSelected ? 'selected' : ''}" 
        ...
        <div class="project-select-option-info">
          ...
          <span class="project-select-option-count">${docCount} doc${docCount !== 1 ? 's' : ''}</span>
        </div>
  ...
  confirm() {
    if (this.isCreatingNew && this.newProjectName) {
      ...
      const result = dataStore.addDocumentsToProject(this.selectedProjectId, this.documentIds);
 ...
 * @returns {AddToProjectModal}
 */
export function getAddToProjectModal() {
  if (!modalInstance) {
    modalInstance = new AddToProjectModal();
  }
